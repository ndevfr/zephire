...
if (estprof()) {
    ...
    if ((!empty($classe)) && (!empty($discipline)) && (!empty($chapitre))) {
        ...
        while ($r = mysqli_fetch_array($result)) {
            $idcompetence[$j]   = $r['id'];
            $desccompetence[$j] = "<strong>" . substr($idcompetence[$j], strlen($niveau) + strlen($discipline)) . "</strong> " . stripslashes($r['nom']);
            if ($r['socle'] != '') {
                $desccompetence[$j] .= " [" . $r['socle'] . "]";
            }
            $j++;
        }
        ...
        foreach ($codes as $cd) {
            echo "<th style='width:60px;'>" . enimage($cd, $classe, $modech) . "</th>";
        }
        for ($j = 0; $j < sizeof($idcompetence); $j++) {
        ...
        echo "<div class='selecteurs'><p>";
        selectcompetence("\"bilanclasse.php?idcl=$classe&amp;idds=$discipline&amp;idch=$chapitre&amp;idcp=\" + this.value");
        echo "</p></div>";
    } else {
    ...
    if ((!empty($classe)) && (!empty($chapitre)) && (!empty($competence))) {
        ...
        while ($el = mysqli_fetch_array($result)) {
            ...
            foreach ($evals as $eval) {
                echo "<td>" . enimage($eval, $classe, $modech) . "</td>";
            }
            if ($modech == 0) {
                echo "<td>" . enimage(estvalide($evals, $autoch), $classe, $modech) . "</td>";
            }
            echo "</tr>";
        }

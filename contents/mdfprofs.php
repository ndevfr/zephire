...
if (estadmin()) {
    ...
    if (!empty($_POST['submit'])) {
        for ($i = 1; $i <= sizeof($_POST['id']); $i++) {
            ...
            if (sizeof($_POST['classespr'][$i]) > 0) {
                $classespr = $link->real_escape_string(implode(",", $_POST['classespr'][$i]));
            } else {
                $classespr = "";
            }
            $disciplinepr = $link->real_escape_string($_POST['discipline'][$i]);
            ...
            if (!empty($passwordpr)) {
                ...
                $sql        = "UPDATE " . $prefix . "profs SET nom = '$nompr', prenom = '$prenompr', classes = '$classespr', discipline = '$disciplinepr', username = '$usernamepr', password = '$passwordpr', admin = $adminpr WHERE id = $idpr";
            } else {
                $sql = "UPDATE " . $prefix . "profs SET nom = '$nompr', prenom = '$prenompr', classes = '$classespr', discipline = '$disciplinepr', username = '$usernamepr', admin = $adminpr WHERE id = $idpr";
            }
            $link->query($sql);
        }
        if (!empty($_POST['nomnew'])) {
            ...
            $disciplinepr = $link->real_escape_string($_POST['disciplinenew']);
            ...
            $sql        = "INSERT INTO " . $prefix . "profs (nom, prenom, classes, discipline, username, password, admin) VALUES ('$nompr', '$prenompr', '$classespr', '$disciplinepr','$usernamepr', '$passwordpr', $adminpr)";
            $link->query($sql);
        }
    }
    if ((!empty($_POST['subUpload'])) && (!empty($_FILES['fichiercsv']['tmp_name']))) {
        ...
        for ($ligne = fgetcsv($fic, 1024, ";"); !feof($fic); $ligne = fgetcsv($fic, 1024, ";")) {
            ...
            if ($symbole !== "#") {
                ...
                $ligne[5] = $link->real_escape_string(encrypt($ligne[5]));
                $ligne[6] = $link->real_escape_string($ligne[6]);
                $sql      = "INSERT INTO " . $prefix . "profs (nom, prenom, classes, discipline, username, password, admin) VALUES ('" . $ligne[0] . "', '" . $ligne[1] . "', '" . $ligne[2] . "', '" . $ligne[3] . "', '" . $ligne[4] . "', " . $ligne[5] . "', " . $ligne[6] . ")";
                $link->query($sql);
            }
        }
        fclose($fic);
    ...
    echo "<thead><tr><th>Nom - Pr&eacute;nom</th><th>Classes</th><th>Discipline</th><th>Identifiant</th><th>Mot de passe</th><th>Admin</th></tr></thead><tbody>";
    ...
    while ($r = mysqli_fetch_array($result)) {
        ...
        echo "<td><select name='classespr[$k][]' type='select' multiple='multiple' size='2'>";
        echo "<option value=''";
        if (count($classespr) == 1) {
        ...
        echo "</select></td>";
        echo "<td><select name='discipline[$k]'><option value=''>Aucune</option>";
    ...
    echo "<td><select name='classesnew[]' type='select' multiple='multiple' size='2'><option value=''>Aucune</option>";
    $sqlclasses = $link->query("SELECT * FROM " . $prefix . "classes ORDER BY niveau DESC, nom ASC");
    while ($c = mysqli_fetch_array($sqlclasses)) {
        $cid  = $c['id'];
        $cnom = $c['nom'];
    ...
    echo "</select></td>";
    echo "<td><select name='disciplinenew'><option value=''>Aucune</option>";
    $sqlclasses = $link->query("SELECT * FROM " . $prefix . "disciplines ORDER BY nom ASC");
    while ($c = mysqli_fetch_array($sqlclasses)) {
        $cid  = $c['id'];
        $cnom = $c['nom'];
    ...
    echo "<p>Exporter un fichier CSV : <input type='submit' value='Valider' onclick='location.href=\"exportcsv.php?cont=profs\"' name='subExport' /></p>";
} else {

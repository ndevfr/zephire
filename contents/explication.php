...
if ($modech == 0) {
    ...
    foreach ($codes as $cd) {
        if ($cd !== "") {
            echo "<tr><td>" . donnelib($cd, $classe, $modech) . "</td><td style='padding-left:5px;padding-right:5px;'>" . enimage($cd, $classe, $modech) . "</td><td style='text-align:left;text-align:left;width:100%;'>" . donnedesc($cd, $classe, $modech);
            if ($notech) {
                echo " J'obtiens " . donnenota($cd, $classe) . " % des points.";
            }
            echo "</td></tr>";
        }
    }
    ...
    for ($i = 0; $i < sizeof($comp); $i++) {
        echo "<tr><td colspan=2 style='text-align:left;background-color:#CCCCCC;'><strong><em>" . substr($comp[$i]['id'], strlen($niveau) + strlen($discipline)) . " : " . $comp[$i]['nom'] . "</em></strong></td></tr>";
        $j = 0;
        if ($modech == 1) {
        ...
        foreach ($ceinturesnv as $cnt) {
            if (in_array($cnt, $ceinturesform)) {
                ...
                echo "<tr><td style='padding-left:5px;padding-right:5px;'>" . enimage($cnt, $classe, 1) . "</td><td style='text-align:left;width:100%;'>" . $txt . "</td></tr>";
            }
            $j++;
        }
    }

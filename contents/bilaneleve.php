...
if ( estprof() ) {
	...
	selecteleve( "\"bilaneleve.php?idcl=$classe&amp;idds=$discipline&amp;idch=$idchapitre&amp;idel=\" + this.value" );
}
...
if ( $eleve !== -1 ) {
	...
	if ( ( !empty( $classe ) ) && ( !empty( $discipline ) ) && ( !empty( $chapitre ) ) && ( !empty( $eleve ) ) ) {
		...
		if ( $modech == 0 ) {
			echo "<p style='text-align:left;'><strong>BILAN DU CHAPITRE : </strong>" . enimage( bilanch( $eleve, $chapitre ), $classe, $discipline, 0 ) . "</p>";
		}
		if ( ( !estprof() ) && ( $autoch == 1 ) ) {
			if ( modeav() ) {
				echo "<p class='noprint'><input type='button' value='Simple' onClick='location.href=\"bilaneleve.php?idcl=" . $classe . "&idds=" . $discipline . "&amp;idch=" . $idchapitre . "&amp;idel=" . $eleve . "\"' />";
			} else {
				echo "<p class='noprint'><input type='button' value='Avanc&eacute;' onClick='location.href=\"bilaneleve.php?idcl=" . $classe . "&idds=" . $discipline . "&amp;idch=" . $idchapitre . "&amp;idel=" . $eleve . "&amp;av=1\"' />";
			}

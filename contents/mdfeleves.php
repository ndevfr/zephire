...
if ( ( estprof() ) ) {
	...
	if ( ( !empty( $_POST[ 'subUpload' ] ) ) && ( !empty( $_FILES[ 'fichiercsv' ][ 'tmp_name' ] ) ) ) {
		$result = $link->query( "SELECT * FROM " . $prefix . "eleves WHERE classe = '$classe'" );
	...
	if ( !empty( $classe ) ) {
		...
		echo "<p class='noprint'>Exporter un fichier CSV : <input type='submit' value='T&eacute;l&eacute;charger' onclick='location.href=\"exportcsv.php?cont=eleves&amp;idcl=" . $classe . "\"' name='subExport' /></p>";
	} else {

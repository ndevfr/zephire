...
if (estprof()) {
    ...
    if (!empty($classe) && !empty($discipline)) {
        $result              = $link->query("SELECT * FROM " . $prefix . "chapitres WHERE id LIKE '$classe%' AND date <> '0000-00-00' AND trimestre <> 0 AND mode <> 1 ORDER BY id");
        $chapitres           = array();
        $numschapitres       = array();
        $nomschapitres       = array();
        ...
        while ($ch = mysqli_fetch_array($result)) {
            $chapitres[$k]           = $ch['id'];
            $numschapitres[$k]       = substr($ch['id'], strlen($classe) + strlen($discipline));

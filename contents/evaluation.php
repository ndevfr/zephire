...
if (estprof()) {
    ...
    selecteleve("\"evaluation.php?idcl=$classe&amp;idds=$discipline&amp;idch=$chapitre&amp;idel=\" + this.value");
    echo "</p></div>";
    function majcps()
    {
    ...
    if ((!empty($classe)) && (!empty($discipline)) && (!empty($chapitre)) && (!empty($eleve))) {
        $tabindex = 6;
        $nbcomps  = sizeof($comp);
        echo "<form action='evaluation.php?idcl=" . $classe . "&amp;idds=" . $discipline . "&amp;idch=" . $chapitre . "&amp;idel=" . $eleve . "' method='POST'>";
        ...
        if ($modech == 0) {
            echo "<p style='text-align:left;'><strong>BILAN AUTO : </strong>" . enimage(bilanauto($eleve, $chapitre), $classe, 0) . "<br /><strong>BILAN PROF : </strong><select id='selectbilan' name='selectbilan'>";
            foreach ($codes as $cd) {
                echo "<option value='$cd'";
        ...
        if (modeav()) {
            echo "<p class='noprint'><input type='button' value='Simple' onClick='location.href=\"evaluation.php?idcl=" . $classe . "&amp;idds=" . $discipline . "&amp;idch=" . $chapitre . "&amp;idel=" . $eleve . "\"' />";
        } else {
            echo "<p class='noprint'><input type='button' value='Avanc&eacute;' onClick='location.href=\"evaluation.php?idcl=" . $classe . "&amp;idds=" . $discipline . "&amp;idch=" . $chapitre . "&amp;idel=" . $eleve . "&amp;av=1\"' />";
        }
